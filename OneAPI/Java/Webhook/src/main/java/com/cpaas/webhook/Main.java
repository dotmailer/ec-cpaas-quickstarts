/*
 * A basic example of receiving data from Engagement Cloud CPaaS using webhooks
 */
package com.cpaas.webhook;
...
public class Main {
    ...
    public ResponseEntity<String> indexPost(@RequestBody String raw_body, @RequestHeader("x-comapi-signature") String request_hmac) {
        // Process data receivied to ensure it is from Engagement Cloud CPaaS and hasn't been tampered with.
        ...
        if (request_hmac.compareToIgnoreCase(hash) != 0) // The request is not from Engagement Cloud CPaaS or has been tampered with
        {
